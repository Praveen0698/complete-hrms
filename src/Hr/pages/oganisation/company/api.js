...
export const saveCompany = async (formData) => {
  try {
    await axios.post("http://localhost:5000/company/create/company", formData, {
      headers: {
...
export const loadCompanyById = async (id) => {
  try {
    const result = await axios.get(`http://localhost:5000/company/get/${id}`, {
      validateStatus: () => {
        return true;
      },
    });
...
export const loadCompany = async () => {
  try {
    const result = await axios.get(
      "http://localhost:5000/company/get/company",
      {
        validateStatus: () => {
          return true;
        },
...
export const updateCompany = async (formData, id) => {
  try {
    const result = await axios.patch(
      `http://localhost:5000/company/update/${id}`,
      formData
    );
